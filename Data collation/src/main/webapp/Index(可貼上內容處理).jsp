...
<body>
    <div class="container">
        ...
        <div class="input-tabs">
            <button id="fileTabBtn" class="input-tab active">上傳檔案</button>
            <button id="pasteTabBtn" class="input-tab">貼上內容</button>
        </div>
        
        <!-- 檔案上傳區塊 -->
        <div id="fileInputOption" class="input-option active">
        ...
        <!-- 貼上內容區塊 -->
        <div id="pasteInputOption" class="input-option">
            <div class="paste-section">
                <h2>1. 貼上資料內容</h2>
                <div class="info-panel">
                    請直接從Excel或CSV檔案中複製資料，並貼到下方文本框中。
                </div>
                <textarea id="pasteArea" class="paste-textarea" placeholder="請在此處貼上Excel或CSV內容..."></textarea>
                <div style="margin-top: 10px;">
                    <button id="parsePastedBtn">解析貼上內容</button>
                </div>
                <div class="progress-container" id="pasteProgressContainer">
                    <div class="progress-bar" id="pasteProgressBar"></div>
                </div>
                <div id="pasteErrorMessage" class="error-message"></div>
                <div id="pasteSuccessMessage" class="success-message"></div>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // DOM 元素 - 新增的元素
        var fileTabBtn = document.getElementById('fileTabBtn');
        var pasteTabBtn = document.getElementById('pasteTabBtn');
        var fileInputOption = document.getElementById('fileInputOption');
        var pasteInputOption = document.getElementById('pasteInputOption');
        var pasteArea = document.getElementById('pasteArea');
        var parsePastedBtn = document.getElementById('parsePastedBtn');
        var pasteProgressContainer = document.getElementById('pasteProgressContainer');
        var pasteProgressBar = document.getElementById('pasteProgressBar');
        var pasteErrorMessage = document.getElementById('pasteErrorMessage');
        var pasteSuccessMessage = document.getElementById('pasteSuccessMessage');
        ...
        fileTabBtn.addEventListener('click', function() {
            ...
            fileInputOption.classList.add('active');
            pasteInputOption.classList.remove('active');
        });
        
        pasteTabBtn.addEventListener('click', function() {
            pasteTabBtn.classList.add('active');
            fileTabBtn.classList.remove('active');
            pasteInputOption.classList.add('active');
            fileInputOption.classList.remove('active');
        });
        ...
        parsePastedBtn.addEventListener('click', function() {
            console.log('解析貼上內容按鈕點擊');
            var content = pasteArea.value.trim();
            if (!content) {
                showPasteError('請先貼上內容');
                return;
            }
            
            hidePasteError();
            hidePasteSuccess();
            showPasteProgressBar();
            
            try {
                // 設置預設檔案年月
                // 尋找年月資訊
                var yearMonthRegex = /中華民國(\d+)年(\d+)月/;
                var yearMonthMatch = content.match(yearMonthRegex);
                
                if (yearMonthMatch) {
                    fileYear = yearMonthMatch[1]; // 民國年
                    fileMonth = yearMonthMatch[2];
                    console.log('從內容提取到年月：民國' + fileYear + '年' + fileMonth + '月');
                } else {
                    // 如果無法從內容中提取年月，使用當前日期作為預設值
                    var now = new Date();
                    var currentYear = now.getFullYear() - 1911; // 轉換為民國年
                    var currentMonth = now.getMonth() + 1;
                    fileYear = currentYear.toString();
                    fileMonth = currentMonth.toString();
                    console.log('無法從內容提取年月，使用當前日期：民國' + fileYear + '年' + fileMonth + '月');
                }
                
                // 開始解析
                updatePasteProgressBar(30);
                ...
                console.log('處理後的CSV數據行數:', csvData.length);
                
                // 更新進度條
                updatePasteProgressBar(50);
                
                // 進行內容解析
                if (csvData.length > 0) {
                    console.log('開始預處理CSV數據');
                    // 預處理CSV數據
                    csvData = preprocessCSVData(csvData);
                    
                    console.log('顯示預覽');
                    // 顯示原始與解除合併後的預覽 (對於貼上的內容，兩者相同)
                    displayCSVOriginalPreview(csvData);
                    displayCSVUnmergedPreview(csvData);
                    
                    console.log('開始提取結構化數據');
                    // 提取資料
                    var extractedData = extractDataFromSpecificFormat(csvData);
                    ...
                    // 設置成功訊息
                    showPasteSuccess('成功解析貼上的內容，共提取 ' + extractedData.length + ' 筆記錄');
                } else {
                    throw new Error('無法解析貼上的內容，可能格式不正確');
                }
                
                // 完成
                updatePasteProgressBar(100);
                setTimeout(function() {
                    hidePasteProgressBar();
                }, 500);
                
            } catch (error) {
                console.error('解析貼上內容時發生錯誤:', error);
                showPasteError('解析貼上內容時發生錯誤: ' + error.message);
                hidePasteProgressBar();
            }
        });
        ...
        function showPasteError(message) {
            pasteErrorMessage.textContent = message;
            pasteErrorMessage.style.display = 'block';
        }
        
        function hidePasteError() {
            pasteErrorMessage.style.display = 'none';
        }
        
        function showPasteSuccess(message) {
            pasteSuccessMessage.textContent = message;
            pasteSuccessMessage.style.display = 'block';
        }
        
        function hidePasteSuccess() {
            pasteSuccessMessage.style.display = 'none';
        }
        
        function showPasteProgressBar() {
            pasteProgressContainer.style.display = 'block';
            updatePasteProgressBar(0);
        }
        
        function hidePasteProgressBar() {
            pasteProgressContainer.style.display = 'none';
        }
        
        function updatePasteProgressBar(percent) {
            pasteProgressBar.style.width = percent + '%';
        }
    });
    </script>
</body>
